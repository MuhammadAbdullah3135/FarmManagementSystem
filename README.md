...
| **Testing** | xUnit, EF Core InMemory, EF Core SQLite (relational, where a real provider is the point), ASP.NET Core `TestWebApplicationFactory` |
...
- **Full-farm export**: one request produces a ZIP of every record the farm holds — one CSV per entity, a manifest stating each file's row count and the exclusions, and the seven entity files that re-import straight back through their own import pages with no column mapping. Built by a background job so a farm with years of history cannot time out the request, and notified in-app when it is ready
...
| Report query counts | `ReportQueryCountTests`, `ReportOutputGoldenTests`, `ReportQueryCountPostgresTests` | Every health/feeding report reads a constant number of times — asserted at two farm sizes on a relational provider and again on PostgreSQL — instead of once per (schedule × animal) pair; each rewritten method's output is compared against a golden recorded from the pre-rewrite implementation, and the batched queries are compiled against Npgsql to prove they still translate |
...
| Bulk import | `SpreadsheetReaderTests`, `AnimalImportServiceTests`, `InventoryImportServiceTests`, `EmployeeImportServiceTests`, `SupplierImportServiceTests`, `CustomerImportServiceTests`, `ExpenseImportServiceTests`, `IncomeImportServiceTests`, `AnimalImportControllerTests`, `ImportControllerTests`, `ImportOptionsBindingTests`, `AnimalImportE2ETests`, `ImportE2ETests` | CSV (including this app's own export shape) and `.xlsx` parsing, header auto-detection and explicit/constant mappings, ambiguous-date refusal, lookup and parent-tag resolution inside and across farms (including a name matching two records), duplicate-identifier and soft-delete parity with each entity's create endpoint in every variant, the deliberate absence of a duplicate rule for expenses and income (identical rows are both written), named-field parity with the endpoint's own messages, the column caps that used to surface as 500s, employee email uniqueness case/padding-insensitively, all-or-nothing commit with a revalidation from the file, file/row-size limits, the legacy `AnimalImport` keys still applying, and farm-context enforcement over HTTP for all seven routes |
...
| Full-farm export | `FarmExportEntityCoverageTests`, `FarmExportAssemblerTests`, `FarmExportIsolationTests`, `FarmExportServiceTests`, `FarmExportNotificationTests`, `FarmExportE2ETests`, `FarmExportLargeFarmTests`, `FarmExportLargeFarmPostgresTests` | Every farm-scoped entity is exported or explicitly excluded with a reason (measured against the EF model, not a hand-kept list), the manifest's row counts equal the files' actual rows, the CSV byte shape the app's own reader accepts, re-importable headers matching each importer's own catalog, a farm's archive containing none of another farm's rows — including child tables with no `FarmId` of their own — the request costing the same regardless of the farm's size, a failed rebuild keeping the previous archive, the completion notice surviving a dispatch run that would have resolved any dashboard alert type, the route's role gate and farm-context enforcement over HTTP, and every exported file re-importing into a second farm unchanged |
